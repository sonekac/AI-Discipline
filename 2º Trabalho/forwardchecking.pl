...
p:- estado_inicial(e(P,B)), restricoes_iniciais(e(P,B), C),
    forwardcheck(e(C,B), A), sort(A), escreve(A),!.

forwardcheck(e([],A),A).
forwardcheck(E, Sol) :- sucessor(E,E1),  propagacao_restricoes(E1, E3),
                        forwardcheck(E3,Sol).
...
linha_arc(_,[], A, A).
linha_arc(v([X,Y],_,Vi), [v([X,Y1],Di,Vj)|R], R1, T) :-
                          !,rem(Vi, Di, Dj),
                          Dj \= [],
                          linha_arc(v([X,Y],_,Vi), R, [v([X,Y1],Dj,Vj)|R1], T).

linha_arc(v([X,Y],_,Vi), [v([X1,Y1],Di,Vj)|R], R1, T) :-
                          linha_arc(v([X,Y],_,Vi), R, [v([X1,Y1],Di,Vj)|R1], T).


coluna_arc(_,[], A, A).
coluna_arc(v([X,Y],_,Vi), [v([X1,Y],Di,Vj)|R], R1, T) :-
                          !, rem(Vi, Di, Dj), Dj \= [],
                          coluna_arc(v([X,Y],_,Vi), R, [v([X1,Y],Dj,Vj)|R1], T).

coluna_arc(v([X,Y],_,Vi), [v([X1,Y1],Di,Vj)|R], R1, T) :-
                         coluna_arc(v([X,Y],_,Vi), R, [v([X1,Y1],Di,Vj)|R1], T).



quadrante_arc(_,[], A, A).
quadrante_arc(v([X,Y],_,Vi), [v([X1,Y1],Di,Vj)|R], R1, T) :-
                      quadrantes(A), member([X,Y],A), member([X1,Y1], A), !,
                      rem(Vi, Di, Dj), Dj \= [],
                      quadrante_arc(v([X,Y],_,Vi), R, [v([X1,Y1],Dj,Vj)|R1], T).

quadrante_arc(v([X,Y],_,Vi), [v([X1,Y1],Di,Vj)|R], R1, T) :-
                      quadrante_arc(v([X,Y],_,Vi), R, [v([X1,Y1],Di,Vj)|R1], T).


escreve([]).
escreve([v([X,9],_,Vi)|Afect]) :- T is X mod 3, T = 0,
                                  write(Vi), nl,write('------+-------+------'),
                                  nl ,escreve(Afect).

escreve([v([_,9],_,Vi)|Afect]) :- write(Vi), nl ,escreve(Afect).

escreve([v([_,Y],_,Vi)|Afect]) :- T is Y mod 3, T = 0,
                                  write(Vi),write(' | ') ,escreve(Afect).
